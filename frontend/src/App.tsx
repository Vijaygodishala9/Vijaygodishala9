...
// Commentary language options - English only
...
// ─── useSpeech ────────────────────────────────────────────────────────────────
// TTS hook - handles text-to-speech for commentary
// Note: Only English voices are reliably available in browsers; Indian language TTS is disabled
...
const ONBOARDING_STEPS = [
  ...
  },
  {
    ...
    body: "Casual hype for drama, Stats for data nerds, or one of 7 Indian languages — the feed speaks your way.",
    example: { label: "SIX", text: "Kohli ne aasman chhoo liya! Zabardast CHHAKKA — jawab nahi!", persona: "हिन्दी" },
  },
  {
    icon: "🔔",
...
export default function App() {
  ...
  // TTS disabled - keeping hook for potential future use
  ...
  useEffect(() => {
    ...
    es.addEventListener("token", (e) => {
      ...
      // Empty token = server signalling a new ball event is starting
      if (token === "") {
      ...
      // Regular token — append to the active in-progress line
      ...
      linesRef.current = [
        ...linesRef.current.slice(0, idx),
        updated,
        ...linesRef.current.slice(idx + 1),
      ];
      setLines([...linesRef.current]);

      if (done) {
        // TTS disabled - uncomment below to re-enable
        // if (TTS_PERSONAS.has(personaRef.current)) speak(newText);
        currentId.current = null;
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 520, margin: "0 auto" }}>
        ...
        <div style={{ display:"flex", gap:0, marginBottom:0, borderBottom:"1px solid rgba(255,255,255,.08)" }}>
          ...
          {/* TTS speaking indicator - disabled */}
          {/* {speaking && TTS_PERSONAS.has(persona) && ( */}
          {/* )} */}
        </div>
        ...
        {activeTab === "scorecard" && (
          <div style={{
            ...
            <div style={{ padding:"14px 16px", borderBottom:"1px solid rgba(255,255,255,.07)" }}>
              <div style={{ fontSize:11, color:"#5A6478", letterSpacing:".07em", textTransform:"uppercase", marginBottom:10 }}>Batting</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 36px 36px 24px 24px 48px", gap:4, marginBottom:6 }}>
                {["Batter","R","B","4s","6s","SR"].map(h => (
                  <div key={h} style={{ fontSize:10, color:"#3A4050", textTransform:"uppercase", letterSpacing:".06em", textAlign:h==="Batter"?"left":"right" }}>{h}</div>
                ))}
              </div>
              {[
                { name:"Virat Kohli *",    runs:45, balls:32, fours:5, sixes:2 },
                { name:"Suryakumar Yadav", runs:38, balls:22, fours:4, sixes:2 },
              ].map((b, i) => {
                const sr = ((b.runs/b.balls)*100).toFixed(1);
                return (
                  <div key={i} style={{ display:"grid", gridTemplateColumns:"1fr 36px 36px 24px 24px 48px", gap:4, padding:"7px 0", borderTop:"1px solid rgba(255,255,255,.05)" }}>
                    <div style={{ fontSize:13, color: i===0?"#F0F4FF":"#8892A4", fontWeight:i===0?600:400, display:"flex", alignItems:"center", gap:6 }}>
                      {i===0 && <span style={{ width:5, height:5, borderRadius:"50%", background:"#4ECCA3", flexShrink:0, animation:"dotBlink 1.4s ease infinite", display:"inline-block" }}/>}
                      {b.name}
                    </div>
                    {[b.runs,b.balls,b.fours,b.sixes].map((v,vi) => (
                      <div key={vi} style={{ textAlign:"right", fontSize:13, fontFamily:"'DM Mono',monospace", color:vi===0?"#F0F4FF":"#5A6478", fontWeight:vi===0?600:400 }}>{v}</div>
                    ))}
                    <div style={{ textAlign:"right", fontSize:12, fontFamily:"'DM Mono',monospace", color:parseFloat(sr)>=150?"#4ECCA3":parseFloat(sr)>=100?"#F0B942":"#5A6478" }}>{sr}</div>
                  </div>
                );
              })}
            </div>
            <div style={{ padding:"14px 16px" }}>
              <div style={{ fontSize:11, color:"#5A6478", letterSpacing:".07em", textTransform:"uppercase", marginBottom:10 }}>Bowling</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 40px 32px 26px 48px", gap:4, marginBottom:6 }}>
                {["Bowler","O","R","W","Eco"].map(h => (
                  <div key={h} style={{ fontSize:10, color:"#3A4050", textTransform:"uppercase", letterSpacing:".06em", textAlign:h==="Bowler"?"left":"right" }}>{h}</div>
                ))}
              </div>
              {[
                { name:"Shaheen Afridi", overs:"4.0", runs:24, wickets:2, eco:"6.00" },
                { name:"Haris Rauf",     overs:"3.3", runs:38, wickets:0, eco:"10.8" },
              ].map((b, i) => (
                <div key={i} style={{ display:"grid", gridTemplateColumns:"1fr 40px 32px 26px 48px", gap:4, padding:"7px 0", borderTop:"1px solid rgba(255,255,255,.05)" }}>
                  <span style={{ fontSize:13, color:b.wickets>0?"#F0F4FF":"#8892A4", fontWeight:b.wickets>0?600:400 }}>{b.name}</span>
                  <div style={{ textAlign:"right", fontSize:13, fontFamily:"'DM Mono',monospace", color:"#5A6478" }}>{b.overs}</div>
                  <div style={{ textAlign:"right", fontSize:13, fontFamily:"'DM Mono',monospace", color:"#5A6478" }}>{b.runs}</div>
                  ...
                  <div style={{ textAlign:"right", fontSize:12, fontFamily:"'DM Mono',monospace", color:parseFloat(b.eco)<=7?"#4ECCA3":parseFloat(b.eco)<=10?"#F0B942":"#E24B4A" }}>{b.eco}</div>
                </div>
              ))}
            </div>
          </div>
